...
class GitHubScraper:
    ...
    def _make_request(self, url: str, params: dict = None, max_retries: int = 10) -> Dict:
        """
        ...
        while attempt < max_retries:
            try:
                ...
                attempt += 1
                self.logger.warning(f"Request timed out. Attempt {attempt} of {max_retries}. Retrying in {2 ** attempt} seconds...")
                time.sleep(2 ** attempt)  # Exponential backoff
            except requests.exceptions.RequestException as e:
                self.logger.error(f"Request failed: {e}")
                break  # If another type of request exception occurs, break the loop
